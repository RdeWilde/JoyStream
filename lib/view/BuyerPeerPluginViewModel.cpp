...
BuyerPeerPluginViewModel::BuyerPeerPluginViewModel(BuyerTorrentPluginViewModel * parent, const libtorrent::tcp::endpoint & endPoint, QStandardItemModel * model)
    : QObject(parent)
    , _endPoint(endPoint)
    , _hostItem(new QStandardItem())
    , _stateItem(new QStandardItem())
    , _payorSlotItem(new QStandardItem())
...
void BuyerPeerPluginViewModel::update(const BuyerPeerPlugin::Status & peerStatus, const Payor::Channel::Status & channelStatus) {

    // Get state
    BuyerPeerPlugin::ClientState clientState = peerStatus.clientState();

    NOT DONE!!!!!!!!!!!!

    // Update other fields if state has moved sufficiently far along
    if(clientState == BuyerPeerPlugin::ClientState::signed_refund ||
       clientState == BuyerPeerPlugin::ClientState::ignored_sign_refund_invitation ||
       clientState == BuyerPeerPlugin::ClientState::awaiting_fullpiece_request_after_ready_announced ||
       clientState == BuyerPeerPlugin::ClientState::awaiting_payment ||
       clientState == BuyerPeerPlugin::ClientState::awaiting_piece_request_after_payment ||
       clientState == BuyerPeerPlugin::ClientState::reading_piece_from_disk) {

        const Payee::Status & payeeStatus = peerStatus.
...
void BuyerPeerPluginViewModel::updatePayorSlotItem(quint32 payorSlot) {

}

void BuyerPeerPluginViewModel::updateLockTime(const QDateTime & lockTime) {

}

void BuyerPeerPluginViewModel::updateFunds(quint64 funds) {

}

void BuyerPeerPluginViewModel::updatePrice(quint64 price) {

}

void BuyerPeerPluginViewModel::updateNumberOfPaymentsMadeItem(quint64 numPayments) {

}

void BuyerPeerPluginViewModel::updateBalanceItem(quint64 balance) {
