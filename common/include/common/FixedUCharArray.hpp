...
#ifndef FIXED_UCHAR_ARRAY_HPP
#define FIXED_UCHAR_ARRAY_HPP
...
class FixedUCharArray : public std::array<unsigned char, array_length> {
...
public:
    ...
    FixedUCharArray(const uchar_vector & vector);

    // Constructor from hex encoded string
    FixedUCharArray(const QString & hexEncoded);

    // Constructor from raw byte array
    FixedUCharArray(const QByteArray & byteArray);
    ...
    // Stream operator
    friend QDataStream & operator<<(QDataStream & stream, const FixedUCharArray & o);
    friend QDataStream & operator>>(QDataStream & stream, FixedUCharArray & o);
...
template<unsigned int array_length>
uint qHash(const FixedUCharArray<array_length> & array);
