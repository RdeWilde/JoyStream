...
#include <extension/BuyerTorrentPluginConfiguration.hpp>o
...
namespace extension {
    ...
    BuyerTorrentPlugin::BuyerTorrentPlugin(Plugin * plugin,
        ...
        : TorrentPlugin(plugin, torrent, bep10ClientIdentifier, configuration, category)
        , _state(BuyerTorrentPluginState::waiting_for_payor_to_be_ready)
        , _wallet(wallet)
        ...
        quint64 contractFee = joystream::paymentchannel::Payor::computeContractFee(configuration.numberOfSellers(), _maxFeePerKb);
        ...
        std::vector<joystream::paymentchannel::ChannelConfiguration> channelConfigurations;
        for(int i = 0;i < configuration.numberOfSellers();i++)
            channelConfigurations.push_back(joystream::paymentchannel::ChannelConfiguration(i,
                                                          joystream::paymentchannel::ChannelState::unassigned,
                                                          0,
                                                          0,
                                                          fundingPerSeller,
                                                          buyerInContractKeys[i],
                                                          buyerFinalKeys[i],
                                                          Coin::PublicKey(),
                                                          Coin::PublicKey(),
                                                          Coin::Signature(),
                                                          ...
                                                          0));
        // Payor configuration
        joystream::paymentchannel::PayorConfiguration payorConfiguration(_wallet->network(),
                                                                        joystream::paymentchannel::PayorState::waiting_for_full_set_of_sellers,
                                                                        ...
                                                                        0);
        // Set payor
        _payor = joystream::paymentchannel::Payor(payorConfiguration);
    ...
    boost::shared_ptr<libtorrent::peer_plugin> BuyerTorrentPlugin::new_connection(libtorrent::peer_connection * peerConnection) {
        ...
        if(!TorrentPlugin::isPeerWellBehaved(peerConnection)) {
            ...
            peerPlugin = new BuyerPeerPlugin(this, btConnection, _bep10ClientIdentifier, true, _category);
            ...
            peerPlugin = new BuyerPeerPlugin(this, btConnection, _bep10ClientIdentifier, false, _category);

            // Alert that peer was added
            // temporary suspension
            //sendTorrentPluginAlert(alert::BuyerPeerAddedAlert(_torrent->info_hash(), endPoint, peerPlugin->status()));
        }
    ...
    void BuyerTorrentPlugin::on_piece_pass(int index) {
        ...
        if(_state != BuyerTorrentPluginState::downloading_pieces) {
        ...
        if(peer->clientState() == BuyerPeerPluginClientState::waiting_for_libtorrent_to_validate_piece) {

            // Make a payment
            Coin::Signature paymentSignature = makePaymentAndGetPaymentSignature(peer);
            peer->sendExtendedMessage(joystream::protocol::Payment(paymentSignature));
            ...
            peer->setClientState(BuyerPeerPluginClientState::needs_to_be_assigned_piece);
    ...
    void BuyerTorrentPlugin::tick() {
        ...
        if(_state == BuyerTorrentPluginState::downloading_pieces) {
            ...
            while(_numberOfUnassignedPieces > 0 &&
                ...
                Q_ASSERT(peerPlugin->clientState() == BuyerPeerPluginClientState::needs_to_be_assigned_piece);
        ...
        // temporary suspension
        //sendTorrentPluginAlert(alert::BuyerTorrentPluginStatusAlert(_torrent->info_hash(), status()));
    }
    ...
    */
    ...
    bool BuyerTorrentPlugin::sellerWantsToJoinContract(BuyerPeerPlugin * peer, quint64 price, quint32 refundLockTime, const Coin::PublicKey & contractPk, const Coin::PublicKey & finalPk) {

        // Check payor is trying to find sellers
        if(_payor.state() != joystream::paymentchannel::PayorState::waiting_for_full_set_of_sellers) {

            // Update peer plugin client state
            peer->setClientState(BuyerPeerPluginClientState::ignored_join_contract_from_peer);
        ...
        Q_ASSERT(_state == BuyerTorrentPluginState::waiting_for_payor_to_be_ready);
        ...
        if(_payor.isFull()) {
            ...
            // Get channels
            const std::vector<joystream::paymentchannel::Payor::Channel> & channels = _payor.channels();

            // Iterate
            quint32 index = 0;
            Coin::TransactionId contractHash = Coin::TransactionId::fromTx(_payor.contractTransaction());

            for(std::vector<joystream::paymentchannel::Payor::Channel>::const_iterator i = channels.cbegin(),
                end = channels.cend(); i != end;i++, index++) {

                // Get channel
                const joystream::paymentchannel::Payor::Channel & channel = *i;

                // isContractFull() =>
                Q_ASSERT(channel.state() == joystream::paymentchannel::ChannelState::assigned);

                // Create sign_refund message
                joystream::protocol::SignRefund m(contractHash, index, channel.funds(), channel.payorContractKeyPair().pk(), channel.payorFinalKeyPair().pk());
                ...
                // =>
                Q_ASSERT(channelPeer->clientState() == BuyerPeerPluginClientState::invited_to_contract);
                //Q_ASSERT(channelPeer->peerState().lastAction() == BuyerPeerPlugin::PeerState::LastValidAction::joined_contract);
                Q_ASSERT(channelPeer->peerState().failureMode() == BuyerPeerPluginPeerState::FailureMode::not_failed);

                // Have plugin send message
                channelPeer->sendExtendedMessage(m);

                // Update peer plugin client state
                channelPeer->setClientState(BuyerPeerPluginClientState::asked_for_refund_signature);
            }

            // Update payor state
            _payor.setState(joystream::paymentchannel::PayorState::waiting_for_full_set_of_refund_signatures);
        }

        return true;
    }

    bool BuyerTorrentPlugin::sellerProvidedRefundSignature(BuyerPeerPlugin * peer, const Coin::Signature & refundSignature) {

        // call conditions =>
        Q_ASSERT(_state == BuyerTorrentPluginState::waiting_for_payor_to_be_ready);
        Q_ASSERT(peer->clientState() == BuyerPeerPluginClientState::asked_for_refund_signature);
        ...
        if(!wasValid) {
            ...
            peer->setClientState(BuyerPeerPluginClientState::received_valid_refund_signature_and_waiting_for_others);
        ...
        if(_payor.allRefundsSigned()) {
            ...
            for(std::vector<BuyerPeerPlugin *>::iterator i = _slotToPluginMapping.begin(), end = _slotToPluginMapping.end(); i != end;i++) {
                ...
                Q_ASSERT(p->clientState() == BuyerPeerPluginClientState::received_valid_refund_signature_and_waiting_for_others);
                //Q_ASSERT(p->peerState().lastAction() == BuyerPeerPlugin::PeerState::LastValidAction::signed_refund);

                // Send ready message
                p->sendExtendedMessage(joystream::protocol::Ready());

                // Update client state
                //p->setClientState(BuyerPeerPlugin::ClientState::announced_ready);
                p->setClientState(BuyerPeerPluginClientState::needs_to_be_assigned_piece);
            ...
            // Update state
            _state = BuyerTorrentPluginState::downloading_pieces;
    ...
    bool BuyerTorrentPlugin::assignPieceToPeerPlugin(BuyerPeerPlugin * peerPlugin) {
        ...
        try {
         ...
         // Send request
         peerPlugin->sendExtendedMessage(joystream::protocol::RequestFullPiece(pieceIndex));

         // Update peer plugin
         peerPlugin->setClientState(BuyerPeerPluginClientState::waiting_for_full_piece);
    ...
    Coin::Signature BuyerTorrentPlugin::makePaymentAndGetPaymentSignature(BuyerPeerPlugin * peerPlugin) {
    ...
    quint64 BuyerTorrentPlugin::totalSentSinceStart() const {
        ...
        for(int i = 0; i < numberOfChannels;i++) {
            ...
            // Get channel
            const joystream::paymentchannel::Payor::Channel & channel = _payor.channel(i);
    ...
    quint64 BuyerTorrentPlugin::channelBalance(int i) const {
        ...
        const joystream::paymentchannel::Payor::Channel & c = _payor.channel(i);
    ...
    */
    void BuyerTorrentPlugin::on_peer_plugin_disconnect(BuyerPeerPlugin * peerPlugin, libtorrent::error_code const & ec) {
        ...
        if(!peerPlugin->scheduledForDeletingInNextTorrentPluginTick()) {
            ...
            // temporary suspension
            // sendTorrentPluginAlert(alert::BuyerPeerPluginRemovedAlert(_torrent->info_hash(), sharedPtr->endPoint()));
    ...
    BuyerTorrentPlugin::Status BuyerTorrentPlugin::status() const {
        ...
        QMap<libtorrent::tcp::endpoint, BuyerPeerPluginStatus> peerPluginStatuses;
    ...
    */
     ...
     */
     ...
     void BuyerTorrentPlugin::setState(const BuyerTorrentPluginState & state) {
         _state = state;
     }
