...
namespace extension {
    ...
    boost::shared_ptr<libtorrent::peer_plugin> SellerTorrentPlugin::new_connection(libtorrent::peer_connection * peerConnection) {
        ...
        if(!TorrentPlugin::isPeerWellBehaved(peerConnection)) {
            ...
            //sendTorrentPluginAlert(alert::SellerPeerAddedAlert(_torrent->info_hash(), endPoint, peerPlugin->status()));
        }
    ...
    SellerPeerPlugin * SellerTorrentPlugin::createRegularSellerPeerPlugin(const Coin::KeyPair & payeeContractKeys,
        ...
        */
        ...
        return new SellerPeerPlugin(this,
                                    ...
                                    false,
                                    joystream::paymentchannel::PayeeConfiguration(joystream::paymentchannel::PayeeState::waiting_for_payor_information,
                                                         0,
                                                         Coin::Signature(),
                                                         _minLock,
                                                         _minPrice,
    ...
    SellerPeerPlugin * SellerTorrentPlugin::createSellerPeerPluginScheduledForDeletion(libtorrent::bt_peer_connection * connection) {
        ...
        return new SellerPeerPlugin(this,
                                    ...
                                    _bep10ClientIdentifier,
                                    true,
                                    joystream::paymentchannel::PayeeConfiguration(),
                                    0,
                                    _category);
    }
    ...
    void SellerTorrentPlugin::tick() {
        ...
        // sendTorrentPluginAlert(alert::SellerTorrentPluginStatusAlert(_torrent->info_hash(), status()));
    ...
    */
    ...
    void SellerTorrentPlugin::on_peer_plugin_disconnect(SellerPeerPlugin * peerPlugin, libtorrent::error_code const & ec) {
        ...
        if(!peerPlugin->scheduledForDeletingInNextTorrentPluginTick()) {
            ...
            sendTorrentPluginAlert(alert::SellerPeerPluginRemovedAlert(_torrent->info_hash(), sharedPtr->endPoint()));

            // Try to clo
            SellerPeerPluginClientState clientState = peerPlugin->clientState();

            if(clientState == SellerPeerPluginClientState::no_bitswapr_message_sent ||
                    clientState == SellerPeerPluginClientState::seller_mode_announced) {

                qCDebug(_category) << "Releasing unused addresses";

                joystream::paymentchannel::Payee payee = peerPlugin->payee();

                _wallet->releaseKey(payee.payeeContractKeys().sk(), false);

                _wallet->releaseKey(payee.payeePaymentKeys().sk(), true);

            } else if(clientState == SellerPeerPluginClientState::awaiting_payment ||
               clientState == SellerPeerPluginClientState::awaiting_piece_request_after_payment ||
               clientState == SellerPeerPluginClientState::reading_piece_from_disk) {
                ...
                // Alter state
                peerPlugin->setClientState(SellerPeerPluginClientState::trying_to_claim_last_payment);
            }
    ...
    */
    ...
    SellerTorrentPluginConfiguration SellerTorrentPlugin::configuration() const {

        return SellerTorrentPluginConfiguration(enableBanningSets(),
                                                  _minPrice,
                                                  _minLock,
                                                  _minFeePerByte,
                                                  _maxNumberOfSellers,
